...
public class UIWorkerMenu : MonoBehaviour
{
    ...
    {
        // ----- rebuild idle grid -----
        foreach (var go in idleIcons) Destroy(go);
        idleIcons.Clear();
        foreach (var w in idleWorkers)
            idleIcons.Add(SpawnIcon(w, idleGrid));

        // ----- rebuild selected grid -----
        foreach (var go in selIcons) Destroy(go);
        selIcons.Clear();
        foreach (var w in selectedWorkers)
            selIcons.Add(SpawnIcon(w, selGrid));
